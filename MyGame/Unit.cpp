...
bool Unit::CollideWith(const Unit& other) const {
	...
	bool intersectOnX{ (mPosX <= oXleft && oXleft <= mPosX + GetUnitWidth()) ||
						(mPosX <= oXright && oXright <= mPosX + GetUnitWidth()) ||
						(oXleft <= mPosX && mPosX + GetUnitWidth() <= oXright) };
	bool intersectOnY{ (mPosY <= oYbottom && oYbottom <= mPosY + GetUnitHeight()) ||
...
void Unit::Draw(Smiley::Shader& shader){
	Smiley::Renderer::Draw(mImage, mPosX, mPosY, mImage.GetWidth(), mImage.GetHeight(), shader);
}
bool Unit::IsPositionPossible(int newX, int newY) const {
	if (newX < 0) return false;
	if (newY < 0) return false;
	if ((newX + mImage.GetWidth() > 800) || (newY + mImage.GetHeight() > 800)) {
		return false;
	}
	else {
		return true;
...
void Unit::SetPower(const std::string& imageFile, int xPos, int yPos, int speed,Unit::Direction newDirection) {
	mPower = new Unit(imageFile, xPos, yPos, speed);
	mPower->SetDirection(newDirection);
}
void Unit::UsePower(Smiley::Shader& shader) {
	mPower->Draw(shader);
	mPower->UpdatePosition();
}